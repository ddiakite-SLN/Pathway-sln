    ...
    with tab2:
        ...
        if career_sub == "🎯 Match me to careers":
            if not _all_answered:
                ...
                st.markdown("More questions = more accurate results. Same method used by O*NET and CareerExplorer.")
            ...
            if search_career:
                ...
                if not matches:
                    ...
                    st.info("Try: Nurse, Engineer, Teacher, Designer, Analyst, Manager, Developer")
                    st.markdown("**Browse by field:**")
                    ...
                    tools = str(career.get('tech_tools','') or '')
                    skills = str(career.get('top_skills','') or '')

                    zone_map = {1:"No degree needed — start immediately",
                                2:"High school diploma — 0-2 years",
                                3:"Associate's or Bachelor's — 2-4 years",
                                4:"Bachelor's degree — 4 years",
                                5:"Graduate degree — 6+ years"}
                    ...
                    # Recommended majors for this career
                    CAREER_MAJORS = {
                        ...
                        "Education": ["Education (K-12)", "Early Childhood Education", "Special Education", "Educational Psychology"],
                        ...
                        "Management": ["Business Administration", "Management", "MBA", "Organizational Leadership"],
                        "Law & Justice": ["Pre-Law", "Criminal Justice", "Political Science", "Legal Studies"],
                        "Arts & Media": ["Communications", "Journalism", "Graphic Design", "Film Production", "Marketing"],
                        "Science & Research": ["Biology", "Chemistry", "Environmental Science", "Physics", "Neuroscience"],
                        "Criminal Justice": ["Criminal Justice", "Criminology", "Forensic Science", "Law Enforcement"],
                        "Construction & Trades": ["Construction Management", "Civil Engineering Technology", "Architecture"],
                        "Culinary Arts": ["Culinary Arts", "Food Service Management", "Hospitality Management"],
                        "Sports & Fitness": ["Kinesiology", "Exercise Science", "Athletic Training", "Sports Medicine"],
                        "Transportation": ["Aviation", "Logistics", "Transportation Management"],
                        "Agriculture": ["Agriculture", "Environmental Science", "Forestry", "Animal Science"],
                    }
                    rec_majors = CAREER_MAJORS.get(field, [])
                    if rec_majors:
                        st.markdown(f"**📚 Recommended majors to become a {title.split(',')[0]}:**")
                        m_cols = st.columns(len(rec_majors[:4]))
                    ...
                    if len(matches) > 1:
                        st.divider()
                        st.markdown(f"**{len(matches)-1} related roles:**")
                        for m in matches[1:6]:
                            m_sal = int(float(m.get('median_annual',0) or 0))
                            sal_str = f"${m_sal:,}/yr" if m_sal else "N/A"
                            st.caption(f"• {m.get('title','')} — {sal_str}")

                    st.caption(f"Source: O*NET 30.2 · BLS 2024")
            else:
